...
int Dsc_mpi::MakeServerSocket(char *port) {
...
int Dsc_mpi::MakeSocket(char *host, char *port) {
	int s; 			// socket
	int len;	//length
...
int Dsc_mpi::Dsc_rank(){
	
	return rank;
}

int Dsc_mpi::Dsc_size(){
	return number_of_processes;
}

string Dsc_mpi::Dsc_processor_name(){
	
	return process_name;
}

void Dsc_mpi::masterNode(){
	string delimiter= " ";
	int len;
	int len1;
	int error;
	...
	while (1) { 
		...
		for(int i=1;i<5;i++){
			...
			if (pollfds[i].revents & POLLIN) {//if there is someone waiting read send there info out
				...
				char tut[100];
				read(pollfds[i].fd,tut,100);
				len=read(pollfds[i].fd, readInBuf, atoi(tut));
				if(len<=0){ 
					pollfds[i].fd=0;
					pollfds[i].events=0;
					pollfds[i].revents=0;
...
void Dsc_mpi::Dsc_mpi_init(char *file_name, char *Number_of_processes, 
	...
	if(check==0){//phase 0 only happends once --- talks to the deamon to get mpi running
		...
		if (myReadFile.is_open()) {
			...
			while (!myReadFile.eof()) {
				...
				if(i>number_of_processes) break;
				//cout<<output<<endl;
...
void Dsc_mpi::Dsc_mpi_finalize(){
	
	if(rank==0){
	const char *readInBuf="exit";
	char one[100];
	sprintf(one,"%d",4);
	write(head_fd,one,100);
	write(head_fd,readInBuf,4);
	}
	//cout<<"closing rank "<<rank<<endl;
	close(head_fd);
	exit(1);
}
int Dsc_mpi::Dsc_mpi_bcast(void* data,int type,int root){
	int er,er1;
	if(rank==root){
		for(int i=0;i<number_of_processes;i++)
			if(i!=root){
				//cout<<"im sending "<<*((int *)data)<<" to "<<i<<endl;
				er=Dsc_mpi_send(data, type,i);
			}
	}
	else{
		er1=Dsc_mpi_revc(data,type,-1);
	}
	if(er==-1||er1==-1)
		return -1;
	return 0;
}
string Dsc_mpi::type(void* data,int type) {//only does ints and strings can add others easy
...
void Dsc_mpi::Dsc_mpi_abort(int rank){
	string temp= "";
	stringstream ss;
	ss << rank;
	...
	temp=temp+ " ABORT ";
	char one[100];
	sprintf(one,"%d",temp.length());
	int er1=write(head_fd,one,100); 
	if(er1==-1){
		close(head_fd);
		cout<<"writing error"<<endl;
		exit(1);
	}
	int er=write(head_fd,temp.c_str(),temp.length());
	if(er==-1){
		close(head_fd);
		cout<<"writing error"<<endl;
		exit(1);
...
int Dsc_mpi::Dsc_mpi_revc(void* data, int type_of_data, int where_from){
	string delimiter= " ";
	...
	if(parsingString=="ABORT"){
		cout<<"Aborting read from rank "<<whereFrom<<endl;
	}	
	else {
...
int Dsc_mpi::Dsc_mpi_send(void* what_your_sending, int type_of_data, int where_to) {
